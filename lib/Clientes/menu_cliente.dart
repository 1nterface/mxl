...
class menu_cliente extends StatefulWidget {
  ...
  final String empresa, correoEmpresa, newid, colonia, calle, numero, tarjeta;
  final int entrada, salida;

  const menu_cliente(this.empresa, this.correoEmpresa, this.newid, this.colonia, this.calle, this.numero, this.entrada, this.salida, this.tarjeta, {Key? key}) : super(key: key);
...
class menu_clienteState extends State<menu_cliente> {
                  ...
                  child: SizedBox(
                    child: RaisedButton(
                      ...
                      onPressed: () async {
                        ...
                        //AQUI ACTIVAR CRONOS de acuerdo al folio del momento, DESPUES DE LOS 10- MINUTOS
                        // /CAMBIAR EL CAMPO QUE LLEVA LA CONDICION EN LA SIGUIENTE PANTALLA
                        //ACERCALO TODOO LOCAL SIN BD
                        cronos(context);
  ...
  @override
  void initState() {
    print("Acepta tarjeta? "+widget.tarjeta);
    print("Acepta tarjeta? "+widget.calle);
    print("Acepta tarjeta? "+widget.colonia);
    print("Acepta tarjeta? "+widget.newid);
    print("Acepta tarjeta? "+widget.correoEmpresa);
    print("Acepta tarjeta? "+widget.empresa);
    print("Acepta tarjeta? "+widget.tarjeta);
    print("Acepta tarjeta? "+widget.tarjeta);
    borrar(context);
  ...
  void cronos(BuildContext context){
    Future.delayed(Duration(seconds: 10), () {
      print("Pasaron 10 minutos, ya no puedes cancelar hijodeputa");
    });
  }
  ...
  Widget recoger (BuildContext context){
    return StreamBuilder<DocumentSnapshot<Object?>>(
      //En esta linea ingresar el nombre de la coecci
        stream: FirebaseFirestore.instance.collection('Socios_Registro').doc(widget.correoEmpresa).snapshots(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return Text("Loading");
          }
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            adomi(context),
            StreamBuilder(
              //ABRIR BAJAFOOD Y VER CUAL ES LA CONDICION YA CON LA LISTA DE RESTAURANTES
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              Row(
                                                ...
                                                children: <Widget>[
                                                  Container(
                                                    child: Text("\$"+documents["costoProducto"].toString(), style: TextStyle(color: Colors.yellow[800], fontSize: 25),),
                                                    //height: 30,
                                                    width: 100,
                                                  ),
                                                ],
