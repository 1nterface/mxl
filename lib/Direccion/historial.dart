...
import 'package:mxl/Direccion/carpinteria.dart';
...
class historialState extends State<historial> {
    ...
    return Expanded(
      child: StreamBuilder(
        //Asi encontraremos los negocios por ciudad y sin problemas con la BD
          stream: reflistaproduccion.where("estadoc", isEqualTo: "Entregado").where('correoNegocio', isEqualTo: correo).orderBy('folio', descending: true).snapshots(),
                                      ...
                                      children: [
                                        Column(
                                          children: [
                                            SizedBox(
                                                width: 120,
                                                child: Text(documents["nombrecliente"]+' #'+documents["folio"].toString(), textAlign: TextAlign.left, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.grey),)),

                                            SizedBox(
                                                width: 120,
                                                child: Text("\$"+documents["totalNota"].toString(), textAlign: TextAlign.left, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.yellow[800]),)),
  ...
  Widget listaHistorialPorFecha(){
    ...
    return Expanded(
        child: StreamBuilder(
          //Asi encontraremos los negocios por ciudad y sin problemas con la BD
            stream: reflistaproduccion.where('miembrodesde', isEqualTo: '${now.year}-${now.month}-${now.day}').where('correoNegocio', isEqualTo: correo).orderBy('folio', descending: true).snapshots(),
            builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
              if (!snapshot.hasData) {
                return Text("Loading..");
              }
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              InkWell(
                ...
                child: Column(
                  children: [
                    Icon(Icons.calendar_today, size: 40, color: Colors.yellow[800]),
