...
class lista_historial_pormes extends StatefulWidget {

  reporte_ventas_porfecha_modelo product;
  lista_historial_pormes(this.product);

  @override
  lista_historial_pormesState createState() => lista_historial_pormesState();
}

class lista_historial_pormesState extends State<lista_historial_pormes> {
  ...
  //COPIAR ESTE METODO Y REEMPLAZARLO POR MES, TMB EN LA BASE DE DATO
  void totalDiario1(){
  ...
  void totalMensual(){
    ...
    FirebaseFirestore.instance.collection('Pedidos_Jimena').where("estadoc", isEqualTo: "Entregado").where('mes', isEqualTo: widget.product.fecha).snapshots().listen((data) async {
      for (var doc in data.docs)  {
        //doc.reference.delete();
        //CREO QUE LA CLAVE ES MANDAR ESTEEste: TOTAL A PEDIDOS_INICIO
    ...
    return Expanded(
      child: StreamBuilder(
        //Asi encontraremos los negocios por ciudad y sin problemas con la BD
          stream: reflistaproduccion.where('estadoc', isEqualTo: "Entregado").where('mes', isEqualTo: widget.product.fecha).where('correoNegocio', isEqualTo: correo).orderBy('folio', descending: true).snapshots(),
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            Column(
              children: [
                const Text("Reporte de ventas mensual", style: TextStyle(color: Colors.white)),
                Text(widget.product.correo, style: const TextStyle(color: Colors.white)),
              ],
            ),
